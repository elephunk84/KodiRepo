...
class tvMaze:
	...
	def episodeAbsoluteNumber(self, thetvdb, season, episode):
		try:
			url = 'http://thetvdb.com/api/%s/series/%s/default/%01d/%01d' % ('MDk1NUY1N0Q5QTlENEZEQw=='.decode('base64'), thetvdb, int(season), int(episode))
	...
	def getTVShowTranslation(self, thetvdb, lang):
		try:
			url = 'http://thetvdb.com/api/%s/series/%s/%s.xml' % ('MDk1NUY1N0Q5QTlENEZEQw=='.decode('base64'), thetvdb, lang)
			r = client.request(url)
